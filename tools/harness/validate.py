    ...
    if latest:
        ...
        if manifest_path.is_file():
            try:
                ...
                match = re.search(r'(?m)^  release:\\s*["\\\']?([0-9]+\\.[0-9]+\\.[0-9]+)["\\\']?\\s*    parser = argparse.ArgumentParser()
...
, manifest_text)
                if match and latest != f"v{match.group(1)}":
                    errors.append(f"update.json latest {latest} does not match manifest harness.release v{match.group(1)}")
            except UnicodeDecodeError:
                pass
