...
8. Сохрани schema-v1 результат в configured `protocol.skillSearchDirectory` по template; не используй hardcoded `planning/skill-searches`. Это позволяет позже выполнить `SKILL INSTALL: #N` без зависимости от истории чата.
