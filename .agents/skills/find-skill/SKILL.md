...
8. Сохрани schema-v1 результат в configured `protocol.skillSearchDirectory` по template; не используй hardcoded `planning/skill-searches`. До завершения проверь конкретный файл: `python3 .harness/tools/report_contract.py --file '<report-path>' --kind skill_search`. Невалидный shortlist нельзя использовать для `SKILL INSTALL: #N`.
